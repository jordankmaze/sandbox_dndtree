      ...
      if (item.type === COLUMN) {
        ...
        newLayoutStructure = {
          ...
          children: [{
            ...
            children: [{ 
              ...item, 
              content: item.content || getDefaultContent(COMPONENT) 
            }]
          }]
        };
      }
...
export const handleAddItemToParent = (layout, parentPath, newItem) => {
  const splitPath = parentPath.split("-");
  // Add item to the end of the parent's children
  const dropZonePath = [...splitPath, "999"]; // Use a high number to add to end
  return addChildToChildren(layout, dropZonePath, newItem);
};

export const handleAddRowToLayout = (layout, newRow) => {
  // Add a new row to the root layout
